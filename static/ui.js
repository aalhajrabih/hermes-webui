...
function renderMd(raw){
  ...
  const parts=s.split(/\n{2,}/);
  s=parts.map(p=>{p=p.trim();if(!p)return '';if(/^<(h[1-6]|ul|ol|table|pre|hr|blockquote)|^\x00[EQ]/.test(p))return p;var hasArabic=/[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/.test(p.replace(/<[^>]+>/g,''));if(hasArabic){return '<p style="text-align:right">'+p.replace(/\n/g,'<br>')+'</p>';}return '<p style="text-align:left">'+p.replace(/\n/g,'<br>')+'</p>';}).join('\n');
  ...
  row.innerHTML=`
    <div class="${cardClass}">
      <div class="tool-card-header"${headerClick}>
        ...
        <span class="tool-card-name" dir="auto"><span class="tool-card-name-label">${esc(displayName)}</span><span class="tool-card-name-generic">${esc(genericName)}</span></span>
        <span class="tool-card-preview" dir="auto">${esc(previewText)}</span>
      ...
      ${hasDetail?`<div class="tool-card-detail" dir="auto">
        ...
        ${displaySnippet?`<div class="tool-card-result" dir="auto">
          <pre dir="auto">${tc.is_diff||_snippetLooksLikeDiff(displaySnippet)?`<code class="diff-block" data-highlighted="1">${_colorDiffLines(displaySnippet)}</code>`:esc(displaySnippet)}</pre>
